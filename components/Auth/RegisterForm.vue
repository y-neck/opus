    ...
    <form
        ...
        <div class="flex flex-col">
            ...
            <transition>
                <p
                    v-if="errorMessage"
                    class="flex text-sm text-grey-500 text-center mt-32 absolute ml-14">
                    {{ errorMessage }}</p>
                <p
                    v-else-if="successMessage"
                    class="flex text-sm text-grey-500 text-center mt-[12.5rem] absolute ml-2">
    ...
    const handleLogin = async() => {
        try {
            ...
            const {error: insertError} = await supabase
                .from('Profiles')
                .insert({email: email.value, password: password.value, user_id: uuid})
