    ...
    <div class="flex items-center gap-2 h-[4.5rem]">
      <div class="-mt-5">
        <input
          ...
          class="checkbox w-4 h-4 border-grey-700 border-[1.5px] rounded cursor-pointer"
        />
        <span v-if="task.status_id === 3" class="absolute -ml-[14.5px] mt-[3px]"
          ><CheckmarkIcon
        /></span>
      </div>
      <div class="flex flex-col">
        ...
        <div class="flex flex-row gap-1 text-grey-500 font-normal">
          ...
          <div v-if="task.assignments && task.assignments.length > 0">
            <p
...
const toggleTaskStatus = async () => {
  const newStatus = task.status_id === 1 ? 3 : 3;
