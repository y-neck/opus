<template>
    ...
    <template #actions>
      ...
      <button class="btn btn-primary border-r border-spacing-2 flex flex-row"> <span class="mt-1"><PlusIcon /></span>  Add Link</button>
      <span class="w-5 h-5 flex justify-center items-center rounded cursor-pointer hover:bg-grey-100 active:text-grey-950 text-grey-500 transition">
                            <DotsIcon />
                        </span>
    </div>
    </template>
      ...
      <main class="p-16">
        <div>
          <div v-if="Object.keys(groupedLinks).length">
            <div class="my-4 mb-9" v-for="(linksGroup, date) in groupedLinks" :key="date">
              <h3 class="font-medium text-sm text-grey-500 tracking-[0.01em]">{{ date }}</h3>
              <hr class="text-grey-100 mt-2">
              <ul>
                <li v-for="(link, index) in linksGroup" :key="index">
                  <div class="flex flex-row my-2">
                    <div class="mr-2 mt-1">
                      <img class="w-4 h-4 rounded" :src="getFaviconUrl(link.link)" alt="favicon">
                    </div>
                    <div class="flex flex-row w-full justify-between group">
                        <div>
                      <a class="font-medium" :href="link.link" target="_blank">{{ formatLink(link.link) }}</a>
                      <p class="font-normal text-grey-500">{{ link.description }}</p>
                    </div>
                      <div class="flex flex-row opacity-0 gap-1 mt-1 group-hover:opacity-100 transition">
                        <span @click="copyDocument(link.link)" class="w-5 h-5 flex justify-center items-center rounded cursor-pointer hover:bg-grey-100 active:text-grey-950 text-grey-500 transition">
                            <CopyIcon />
                        </span>
                        <span class="w-5 h-5 flex justify-center items-center rounded cursor-pointer hover:bg-grey-100 text-grey-500 active:text-grey-950 transition">
                            <PencilIcon />
                        </span>
                        <span @click="showDeleteModal(link)" class="w-5 h-5 flex justify-center items-center rounded cursor-pointer hover:bg-grey-100 text-grey-500 active:text-destructive-red transition">
                            <TrashCanIcon />
                        </span>
                      </div>
                    </div>
...
import Modal from '~/components/Modals/Remove.vue';
  import { ref } from 'vue';
...
async function deleteDocument() {
  try {
    ...
    // Attempt to delete the document from the database using its ID
    const { error, status } = await supabase
      .from('Links')
      .delete()
      .eq('id', documentToDelete.value.id); // Use the correct document ID to target the document
